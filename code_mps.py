...
class Time_Operator:
    ...
    def Calculate_J(self, Lind_Op, site):
        ...
        for i in range(np.shape(Lind_Op)[0]):
            if site == 0:       # First site
                J_right -= np.kron( np.kron(np.eye(self.d), np.matmul(np.conj(np.transpose(Lind_Op[i])), Lind_Op[i])), np.eye(self.d**2) ) /2
                J_right -= np.kron( np.kron(np.eye(self.d), np.matmul(np.transpose(Lind_Op[i]), np.conj(Lind_Op[i]))), np.eye(self.d**2) ) /2
                J_left -= np.kron( np.kron(np.matmul(np.transpose(Lind_Op[i]), np.conj(Lind_Op[i])), np.eye(self.d)), np.eye(self.d**2) ) /2
                J_left -= np.kron( np.kron(np.matmul(np.conj(np.transpose(Lind_Op[i])), Lind_Op[i]), np.eye(self.d)), np.eye(self.d**2) ) /2
            else:               # Last site
                J_right -= np.kron( np.eye(self.d**2), np.kron(np.eye(self.d), np.matmul(np.conj(np.transpose(Lind_Op[i])), Lind_Op[i])) ) /2
                ...
                J_left -= np.kron( np.eye(self.d**2), np.kron(np.matmul(np.transpose(Lind_Op[i]), np.conj(Lind_Op[i])), np.eye(self.d)) ) /2
    ...
    def create_J_m(self, Lind_Op, site, ts, m):
        ...
        for i in range(1,m+1):
            temp_L = np.add(temp_L, np.conj(self.Calculate_J(Lind_Op, site)[0]**i) * abs(ts)**i / math.factorial(i))
    ...
    def Create_F(self, Lind_Op, site, order, m, s_val):
        ...
        for i in range(order):
            temp = np.matmul(self.create_J_m(Lind_Op, site, (s_val[i+1]-s_val[i]), m), temp)
        return temp
    ...
    def Calculate_LL_site(self, Lind_Op, site):
        ...
        temp = np.zeros((self.d**2, self.d**2), dtype=complex)              
        for i in range(np.shape(Lind_Op)[0]):
            temp += np.kron(np.conj(Lind_Op[i]), Lind_Op[i])
        if site == 0:
    ...
    def Approximate_LL(self, Lind_Op, site, dt, order):
        """ Approximates L_L by a Taylor series truncation """
    ...
    def Approximate_LJ(self, Lind_Op, site, dt, order): 
        """ Approximates L_J by a 'Taylor series' truncation in Kraus form """
