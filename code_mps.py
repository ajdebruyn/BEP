# -*- coding: utf-8 -*-
"""
Created on Tue Mar 19 18:43:17 2024
...
class MPS:
    ...
    def TEBD(self, TimeOp, TimeOp_Diss, normalize, Diss_bool):
        """ TEBD algorithm """
        ...
        if Diss_bool:
            ...
            for i in range(len(TimeOp_Diss["index"])):
                self.apply_singlesite(TimeOp_Diss["TimeOp"][i], TimeOp_Diss["index"][i], normalize)
            if self.N%2 == 1:
                self.apply_twosite(TimeOp[self.N-2]/2, self.N-2, normalize)
    ...
    def TEBD_new(self, TimeOp, Lindbladian_arr, normalize, Diss_bool, step):
    ...
    def time_evolution(self, TimeEvol_obj, normalize, steps, desired_expectations, track_normalization, track_energy, track_current):
        ...
        for t in range(steps):
            ...
            self.TEBD_new(TimeOp, TimeOp_Lindblad, normalize, Diss_bool, t)
        ...
        data["Trace_max"][asdf,jkl] = max(self.trace[:int(steps/12.5)])
        ...
        for i in range(self.N):
            ...
            osc_max = max(osc_max, max(Sz_expvals[i,-int(steps/25):]) - min(Sz_expvals[i,-int(steps/25):]))
...
class Time_Operator:
    def __init__(self,N, d, JXY, JZ, h, s_coup, weightLR, dt, Diss_bool, is_density, use_CN):
        self.N = N
        self.d = d
        self.JXY = JXY
        self.JZ = JZ
        ...
        self.TimeOp_Lindblad = self.create_TimeOp_Lindblad(self.weightLR, self.dt, self.use_CN, self.order)
    ...
    def Create_TimeOp(self, dt, use_CN):
        ...
        if use_CN:
            U[0,:,:] = self.create_crank_nicolson(self.Ham[0], -1j*dt)
            U[self.N-2,:,:] = self.create_crank_nicolson(self.Ham[self.N-2], -1j*dt)
            U[1:self.N-2,:,:] *= self.create_crank_nicolson(self.Ham[1], -1j*dt) # we use broadcasting
        else:
    ...
    def create_crank_nicolson(self, H, dt):
        ...
        H_top=np.eye(H.shape[0])+dt*H/2
        H_bot=np.eye(H.shape[0])-dt*H/2
    ...
    def Calculate_J(self, Lind_Op, site):
        ...
        for i in range(np.shape(Lind_Op)[0]):
            if site == 0:
                J_left += np.kron(np.eye(self.d), np.matmul(np.conj(np.transpose(Lind_Op[i])), Lind_Op[i] ))
                J_right += np.kron(np.matmul(np.transpose(Lind_Op[i]), np.conj(Lind_Op[i])), np.eye(self.d))
            else:
                J_left += np.kron(np.eye(self.d), np.matmul(np.conj(np.transpose(Lind_Op[i])), Lind_Op[i] ))
                J_right += np.kron(np.matmul(np.transpose(Lind_Op[i]), np.conj(Lind_Op[i])), np.eye(self.d))
        return J_left, J_right
    
    def create_J_m(self, Lind_Op, site, m, t_val, s_val):
        Op = np.eye(self.d**4)
        for i in range(1,m+1):
            Op = np.add(Op, np.kron(np.conj(self.Calculate_J(Lind_Op, site)[1]**i), self.Calculate_J(Lind_Op, site)[0]**i) * abs(t_val-s_val)**(2*i) / (2 * math.factorial(i)))
        return Op  #self.apply_twosite(Op, site, normalize)
    
    def Create_F(self, Lind_Op, site, order, m, s_val):
        temp = self.create_J_m(Lind_Op, site, m, s_val[0], 0)
        for i in range(order):
            temp = np.matmul(self.create_J_m(Lind_Op, site, m, s_val[i+1], s_val[i]), temp)
        return temp
    ...
    def Calculate_LJ_site(self, Lind_Op, site):
        ...
        if Lind_Op.ndim==2:     #If only a single operator is given, this matrix is used
            temp -= np.kron(np.matmul(np.conj(np.transpose(Lind_Op)), Lind_Op), np.eye(self.d))/2
            temp -= np.kron(np.eye(self.d), np.matmul(np.transpose(Lind_Op), np.conj(Lind_Op)))/2
    ...
    def Approximate_LJ(self, Lind_Op, site, dt, order): 
        ...
        temp = np.eye(self.d**4)
        for j in range(1,order+1):
            temp = np.add(temp, LJ**j * dt**j)
        return temp
    
    def Calculate_L_site(self, Lind_Op, site, dt, order):
        temp = self.Approximate_LJ(Lind_Op, site, dt/2, order)
        temp2 = np.matmul(temp, np.matmul(self.Approximate_LJ(Lind_Op, site, dt, order), temp))
        return np.reshape(temp2, (self.d**2, self.d**2, self.d**2, self.d**2))
    
    def create_TimeOp_Lindblad(self, weightLR, dt, use_CN, order):
    ...
    def create_TimeOp_Lindblad1(self, weightLR, dt, use_CN):
        Lb_arr = np.zeros((), dtype=[
            ("index", int, 2),
            ("Operator", complex, (2, self.d**4, self.d**4)),
            ("TimeOp", complex, (2, self.d**2, self.d**2, self.d**2, self.d**2))
        ...
        for i in range(len(Lb_arr["index"])):
            Lind_Op[i] = self.Lindblad_operators(weightLR[i])
            Lb_arr["Operator"][i,:,:] = self.Calculate_LJ_site(Lind_Op[i], Lb_arr["index"][i]) + self.Calculate_LL_site(Lind_Op[i])
        
        for i in range(len(Lb_arr["index"])):
            if use_CN:
                temp = self.create_crank_nicolson(self.Calculate_LL_site(Lind_Op[i])/2, dt)
                temp2 = np.matmul(temp, self.create_crank_nicolson(self.Calculate_LJ_site(Lind_Op[i], Lb_arr["index"][i]), dt))
                temp = np.matmul(temp2, temp)
                Lb_arr["TimeOp"][i,:,:,:,:] = np.reshape(temp, (self.d**2, self.d**2, self.d**2, self.d**2))
                 
            else:
                temp = expm(dt*Lb_arr["Operator"][i])
...
def Kraus(Op, Obj):
    return np.matmul(Op, np.matmul(Obj, np.conj(np.transpose(Op))))
...
use_CN      = True      #to use Crank-Nicolson approximation
...
def main():
    ...
    TimeEvol_obj1 = Time_Operator(N, d, JXY, JZ, h, s_coup, weightLR, dt, Diss_bool, True, use_CN)
...
t_range = [0.05] # [0.03, 0.04, 0.05, 0.06, 0.07, 0.08, 0.085, 0.09, 0.095]
...
for asdf in range(len(t_range)):
    ...
    for jkl in range(len( chi_range)):
        ...
        filename = 'C:\\Users\\sande\\Downloads\\BEPtest4'
