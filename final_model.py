# -*- coding: utf-8 -*-
"""
Created on Sat Jun 15 22:10:19 2024
...
''' The class MPS performs all calculations regarding MPS, but not time-evolution. '''
''' For more information about certain parameters or functions, see chapter 3 of the thesis. '''
class MPS:
    ...
    def time_evolution(self, TimeEvol_obj, normalize, steps, desired_expectations, track_normalization, track_energy, track_current):
        """ creates the time-evolution operators and performs the time-evolution """
        ...
        if t == steps-1:
            final_Sz = np.zeros(N)
            for i in range(N):
                final_Sz[i] = Sz_expvals[i,-1]
            plt.plot(range(1,N+1), final_Sz, linestyle="", marker=".")
            plt.xticks(range(1,N+1))
            plt.xlabel("Site")
            plt.ylabel("$〈 S_Z 〉$")
            plt.grid()
            plt.title("Expectation value of $〈 S_Z 〉$ for each site") #after {steps+1} steps with dt={dt}")
            plt.show()  
            pass
...
class Time_Operator:
    ...
    def Calculate_J(self, Lind_Op, site):
        """ Calculates J as in Thesis Eq. (4.9) """
    ...
    def create_J_m(self, Lind_Op, site, ts, m):
        """ Approximate exp(L_J (t-s)) by J_m(t-s) as in Thesis Eq. (4.17) """
    ...
    def Create_F(self, Lind_Op, site, order, m, s_val):
        """ Create F_m^M as in Thesis Eq. (4.19) """
        ...
        return np.eye(self.d**4)
    
    def Create_Duhamel(self, Lind_Op, site, order):
        """ Creates a high-dimensional midpoint approximation of the Duhamel notation up to 2nd order as in Thesis Eq. (4.23) """
    ...
    def Create_Duhamel_RK(self, Lind_Op, site, order):
        """ Creates a nested Runge-Kutta approximation of the Duhamel notation """
        # return self.create_J_m(Lind_Op, site, order, 0, self.dt) + self.dt * self.create_J_m(Lind_Op, site, order-1, 0, self.dt) * self.Approximate_LL(Lind_Op, site, order)
        Op = self.create_J_m(Lind_Op, site, self.dt, order)
        if order == 4:
            ...
            print('Only use RK4 with order 4.')
            return
        return Op
    
    def RK4(self, Lind_Op, site, order, m, s_val, k_list, cyc):
        """ Calculate nested RK4 to approximate the integral up to 4th order as in Thesis Alg. (1) """
    ...
    def Approximate_LL(self, Lind_Op, site, dt, order):
        """ Approximates exp(dt L_L) by a Taylor series truncation as in Thesis Eq. (4.1) """
    ...
    def Approximate_LJ(self, Lind_Op, site, dt, order): 
        """ Approximates exp(dt L_J) by a 'Taylor series' truncation in Kraus form as in Thesis Eq. (4.11) """
    ...
    def Calculate_L_site(self, Lind_Op, site, dt, order):
        ''' Calculates the total exp(dt L), second order as in as in Thesis Eq. (4.10) '''
    ...
    def create_TimeOp_Lindblad(self, weightLR, dt, order):
        ...
        for i in range(len(Lb_arr["index"])):
            ...
            if order == 3:
                ...
                print(f"Order {order} is not possible, choose 2 or 3")
                break
...
t_end       = 25        #Length of the time interval
dt          = 0.02      #Time step
order       = 3         #Approximation order of the numerical time integration
...
max_cores   = 6         #Number of cores of the computer
...
weightLR    = np.array([[0,2,0,0],[0,0,2,0]])   #Dissipative factors gamma
d           = 2                                 #Spin dimension
steps       = round(t_end/dt)                   #number of time steps
